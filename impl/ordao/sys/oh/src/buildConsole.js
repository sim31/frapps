...
const dCfg = jsonfile.readFileSync("../../sc-deployment/ignition/deployments/oh/deployed_addresses.json");
const dparams = jsonfile.readFileSync("./deployment-params.json");
const chainInfo = jsonfile.readFileSync("./chain-info.json")

const buildDirFromOrclient = "../ordao/console/build/oh"
const buildDir = "build/oh"

const docCmd = `cd $npm_package_config_orclient && \
  npx typedoc ./src/orclient.ts \
  --name "ORConsole (ORDAO office hours)" \
...
const consoleCmd = `cd $npm_package_config_ordao_console && \
  ...
  VITE_ORNODE_URL=https://ordao-ornode.frapps.xyz \
  VITE_APP_TITLE="ORConsole (ORDAO office hourse)" \
  \
