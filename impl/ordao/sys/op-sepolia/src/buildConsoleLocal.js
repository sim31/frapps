...
const buildDirFromOrclient = "../ordao/console/build/op-sepolia-local"
const buildDir = "build/op-sepolia-local"

const docCmd = `cd $npm_package_config_orclient && \
  npx typedoc ./src/orclient.ts \
  --name "ORConsole (OP Sepolia local)" \
...
const consoleCmd = `cd $npm_package_config_ordao_console && \
  ...
  VITE_APP_TITLE="ORConsole (OP Sepolia local)" \
  \
