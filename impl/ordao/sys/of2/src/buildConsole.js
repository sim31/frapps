...
const dCfg = jsonfile.readFileSync("../../sc-deployment/ignition/deployments/of2/deployed_addresses.json");
const dparams = jsonfile.readFileSync("./deployment-params.json");
const chainInfo = jsonfile.readFileSync("./chain-info.json")

const buildDirFromOrclient = "../ordao/console/build/of2"
const buildDir = "build/of2"

const docCmd = `cd $npm_package_config_orclient && \
  npx typedoc ./src/orclient.ts \
  --name "ORConsole (Optimism Fractal)" \
...
const consoleCmd = `cd $npm_package_config_ordao_console && \
  ...
  VITE_APP_TITLE="ORConsole (Optimism Fractal)" \
  \
